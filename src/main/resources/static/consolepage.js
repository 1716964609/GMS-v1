document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('list-form').addEventListener('submit', async function(e) {
        ...
        if (!listData.listId.length > 0) {
            const lists = await apiCall('/lists');
            document.getElementById('list-id').value = lists[lists.length - 1].listId; 
        } else {
        ...
        if (!listData.listId.length > 0) {
            const lists = await apiCall('/lists');
            document.getElementById('list-for-term-id').value = lists[lists.length - 1].listId; 
        } else {
            document.getElementById('list-for-term-id').value = listData.listId;
        }
        
        document.getElementById('list-for-term-name').value =listData.listName;
        document.getElementById('update-list-btn').disabled = false;
        document.getElementById('delete-list-btn').disabled = false;
        await searchKeyword();
    });
    ...
    document.getElementById('term-form').addEventListener('submit', async function(e) {
        ...
        if (!termData.termId.length > 0) {
            const terms = await apiCall('/terms-in-list', 'POST', { listId: termData.listId });
            document.getElementById('term-id').value = terms[terms.length - 1].termId;
        } else {
    ...
    async function deleteList(listData) {
        if (confirm(`${listData.listName}を削除しますか？`)) {
            try {
                ...
                alert(`${listData.listName}を削除しました。`);
            } catch (error) {
                alert('削除が失敗しました。やり直してください。');
                console.error(error);
            }
        }
    }
